...
默认数据文件为 `dataset/ctr_data_500k.csv`，需要包含：
...
```powershell
python main.py rank train --data-path dataset/ctr_data_500k.csv --epochs 5
```
...
```powershell
python main.py recall train --data-path dataset/ctr_data_500k.csv --epochs 6 --loss-type infonce
```
...
python main.py recall generate `
  ...
  --data-path dataset/ctr_data_500k.csv `
  --top-k 20 `
  --num-users 20
```
